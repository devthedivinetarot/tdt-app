...
function AuthView() {
  ...
  const send = async () => {
    ...
    if (!ok) setErr('OTP bhejne mein dikkat — number check karein (ya web par Google se sign in karein).');
  };
  const verify = async () => {
    setErr('');
  ...
  return (
    <Screen contentStyle={{ justifyContent: 'center', flexGrow: 1 }}>
      ...
      <Text style={styles.demoNote}>Real OTP via SMS · ya Google se sign in karein.</Text>
    </Screen>
  );
}
